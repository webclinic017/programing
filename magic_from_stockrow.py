...
def get_d0(name="O",freq="quarterly",types='cashflow'):
    if freq !='annual' and freq !='quarterly':
        print('freq must be annual , quarterly')
        return 0
    
    tt = types

    res = requests.get('https://stockrow.com/{}/financials/{}/{}'.format(name,tt,freq))
...
#    margin_day=7
    ...
    vv = yf.download(tick , start=now2, end=now)
    ...
    for mo in months:
        ...
        vv2 = yf.download(tick , start=m1, end=m2)
...
#        book = dd0[dd0.index =='Share Holder Equity'][flag].iloc[0,0]
    ...
    aa.extend(ret)
    mem.append(aa)
    print(tick,'\t', ii,'/',len(tickers),'\t',aa)

df1 = pd.DataFrame(mem)
df1.columns=['TICK','GP/A','PBR','1M','3M','6M','12M']
